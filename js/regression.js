...
(function _umd(global, factory) {
  ...
  function mse(data, predicted) {
    ...
    let sum = 0;
    for (let i = 0; i < n; i++) {
        sum += (data[i][1] - predicted[i][1])*(data[i][1] - predicted[i][1]);
    } 

    return (sum/n);
  }

  function gradiant(equation, data, predicted, k, lr) {
    let coeff     = [];
    let sums      = [];
    let n         = data.length;
    ...
    for (let i = 0; i < n; i++) {
        let x     = data[i][0];
        let y     = data[i][1];
        let y_bar = predicted[i][1];
       
        for (let j = 0; j < k; j++) {
            sums[j].push((x**j)*(y-y_bar));
        }       
    }
    sums = sums.reverse();

    for (let i = 0; i < k; i++) {
       coeff[i] = equation[i] - (lr * (-((k-1) / n) * sums[i].reduce((a, b) => a + b, 0))); 
    }
  ...
  var methods = {
    ...
    polynomial: function (data, order, options) {
      ...
      let error     = mse(data, results);
      

      /* 
      let gradiants = [0,0,0];
      for (let i = 0; i < 1500; i++ ){
          var loss = gradiant(gradiants, data, results, k, 0.0001); 
          gradiants = loss;
      }

      var results_a = data.map(function (xyPair) {
        var x = xyPair[0];

        var answer = gradiants.reduce(function (sum, coeff, power) {
          return sum + coeff * Math.pow(x, power);
        }, 0);
